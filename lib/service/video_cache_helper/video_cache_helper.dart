...
class VideoCacheHelper {
  ...
  static Future<FileInfo?> getValidCachedVideo(String url) async {
    ...
    if (timestampStr != null) {
      ...
      if (timestamp != null &&
        ...
        if (cachedFile != null) {
          log('[CACHE] Valid cached video found: ${cachedFile.file.path}');
          return cachedFile;
        }
      }
    }

    log('[CACHE] No valid cache or cache expired for $url');
    return null;
  }
  ...
  static Future<FileInfo> downloadAndCacheVideo(String url) async {
    ...
    log('[DOWNLOAD] Download complete: ${fileInfo.file.path}');
    ...
    try {
      ...
      for (final key in allKeys) {
        ...
        if (file != null) {
          ...
          log('[CLEAR] Removed expired storage key: $key');
        }
      }
      ...
      if (await dir.exists()) {
        ...
        for (final file in allFiles) {
          if (!validPaths.contains(file.path)) {
            await file.delete();
            log('[CLEAR] Deleted unused file: ${file.path}');
          }
        }

        log('[CLEAR] Valid cached paths: ${validPaths.length}');
        log('[CLEAR] Total files deleted: ${allFiles.length - validPaths.length}');
      } else {
        log('[CLEAR] Cache directory does not exist: $cachePath');
      }
    } catch (e) {
      log('[ERROR] clearExpiredVideos: $e');
    }
  }
  ...
  static Future<void> clearAllCache() async {
    try {
      ...
      if (await dir.exists()) {
        ...
        for (final file in files) {
          try {
            ...
            log('[WARNING] Failed to delete file: ${file.path}, error: $e');
          }
        }
        log('[CACHE] Cache directory cleared: $cachePath');
      } else {
        log('[CACHE] Cache directory not found: $cachePath');
      }
      ...
      log('[CACHE] All cache keys cleared');
    } catch (e) {
